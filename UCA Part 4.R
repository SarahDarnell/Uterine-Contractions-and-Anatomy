...
#Written by Sarah Darnell, began 5.15.25, lasted edited 5.27.25
...
##KH requests knew smaller tables, as described below

#table 2c - nonmenses data (eh16 only) for contvars
#Remove menses visits from eh16
uca_filtered_nonmenses <- uca %>%
  filter(menses == 0)

table2c <- uca_filtered_nonmenses %>%
  select(all_of(contvar), t1group) %>%
  filter(!t1group %in% c("Fibroid", "Endometriosis")) %>%
  group_by(t1group) %>%
  summarize(across(everything(), ~ sprintf("%.1f [%.1f-%.1f]", 
...
#kruskal wallis test
uca_filtered_groups <- uca_filtered_nonmenses %>%
  filter(t1group %in% c("Dysmenorrhea", "Pain Free Control"))
...
kw <- lapply(contvar, function(var) {
  ...
  if (nlevels(temp$t1group) < 2 || length(unique(temp[[var]])) < 2) {
...
#kruskal wallis test
uca_filtered_groups <- uca_filtered_menses %>%
  filter(t1group %in% c("Dysmenorrhea", "Pain Free Control"))
...
uca_filtered_menses_pain <- uca_filtered_menses %>%
  filter(
      #EH16, DYS, pain > 2
      (t1group == "Dysmenorrhea" & study == "EH16" &
       (is.na(`Menses Visit: Maximum cramping pain post scan`) |
...
##below here needs to be work shopped for final table 2a##
